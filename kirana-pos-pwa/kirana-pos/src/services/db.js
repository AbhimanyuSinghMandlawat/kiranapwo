...
/* ===== STOCK UPSERT BY NAME (BillScanner duplicate handling) ===== */

/**
 * If an item with the same name already exists → add qty to existing record.
 * If it does NOT exist → create a new stock item.
...
export async function upsertStockByName(item) {
  ...
  return new Promise((resolve, reject) => {
    ...
    check.onsuccess = () => {
      if (check.result) {
        // Existing item → bump quantity
        const existing      = check.result;
        existing.quantity   = (existing.quantity || 0) + (item.quantity || 1);
        // Also update prices if supplied
